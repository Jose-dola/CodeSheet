 ...
 * @author akhrain
 */
public class WindowOption implements Solvable
{
    CodeString text = null;
    String[] options = null;
    Line[]   values  = null;
    
    public WindowOption(String s, CodeSheet cs)
    {
        int len = s.length();
        int j;
        int i = s.indexOf(',');
    ...
    {
        int i = JOptionPane.showOptionDialog(null, text.CodeStringToString(cs, em), "Choose an option", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
        return values[i].solve(cs, em);
    }
}
